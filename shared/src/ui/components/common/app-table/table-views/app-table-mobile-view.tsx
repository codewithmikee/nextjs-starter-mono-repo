...
interface MobileTableViewProps<TData> {
  table: Table<TData>;
  titleKey: string;
  actionKey?: string;
  linkGetter?: (row: TData) => string;
  onClick?: (row: TData) => any;
}

export default function MobileTableView<TData>({
  table,
  titleKey,
  actionKey,
  linkGetter,
  ...
  const CardWrapper = ({
    ...
    if (linkGetter) {
      return (
        <Link href={linkGetter(row)} className='block hover:shadow-lg'>
          {children}
        </Link>
      );
    }
    ...
    <div className='space-y-3 md:hidden'>
      {table.getRowModel().rows.map((row) => {
        ...
        const descriptionCells = row
          ...
          .filter(
            (cell) =>
              (cell.column.columnDef as AppColumnDef<TData>).descriptionColumn
          );
        ...
        const nextToTitle = row
          .getAllCells()
          .filter(
            (cell) => (cell.column.columnDef as AppColumnDef<TData>).nextToTitle
          );

        // Get regular content cells (excluding title, action, and description)
        const contentCells = row.getAllCells().filter((cell) => {
          const colDef = cell.column.columnDef as AppColumnDef<TData>;
          return (
            !colDef.nextToTitle &&
        ...
        return (
          <CardWrapper key={row.id} row={row.original}>
            <Card className='overflow-hidden'>
              <CardHeader className='py-3'>
                <CardTitle className='mb-0 flex items-center justify-between pb-0 text-base'>
                  <span className='flex items-center gap-2 font-medium'>
                    ...
                    {nextToTitle.length > 0 && (
                      <div className='ml-2 mt-1 flex flex-wrap items-center gap-2 text-xs'>
                        {nextToTitle.map((cell) => (
                          <ClickableWrapper key={cell.id}>
                            {flexRender(
                ...
                {descriptionCells.length > 0 && (
                  <div className='mt-0 flex items-center pt-0 text-xs'>
              ...
              {contentCells.length > 0 && (
                <CardContent className='flex w-full gap-2 py-2'>

...
export const renderDesktopFilters = <T,>(
...
export const renderFiltersMobile = <T,>(
  filters: TableFilterConfig<T>[],
  mobileTable: ReactTable<T>,
  mobileConfig: MobileViewConfig<T>,
  sortColumn: string | null,
  setSorting: React.Dispatch<React.SetStateAction<SortingState>>,
  isDescending: boolean,
  handleDirectionToggle: (checked: boolean) => void,
    ...
    <div className='flex w-full flex-col gap-2 py-2'>
      <div className='flex w-full flex-row gap-2'>
        <div className='w-full flex-grow'>
          {renderFilters(
            filters.filter((fl) => fl.type == 'text'),
            mobileTable
          )}
        </div>
        {filters.length > 0 && (
          <Sheet>
            <SheetTrigger className='relative w-fit'>
              {isFilterBySelectSelected.length > 0 && (
                <span className='absolute right-0 top-0 inline-flex h-10 w-10 -translate-y-1/2 translate-x-1/2 transform items-center justify-center rounded-full bg-red-500 text-xs text-white'>
      ...
      {renderMobileSorting(
        mobileConfig.sortableColumns || [],
        sortColumn,
        (columnId) => setSorting([{ id: columnId, desc: isDescending }]),
        isDescending,
...
export const renderMobileSorting = <T,>(
  sortableColumns: { key: keyof T; label: string }[],
  mobileSortColumn: string | null,
  handleMobileSortColumnChange: (columnId: string) => void,
  isDescending: boolean,
  handleDirectionToggle: (checked: boolean) => void
) => {
  if (!sortableColumns || sortableColumns.length === 0) return null;
    ...
    <div className='flex w-full flex-wrap items-center justify-end gap-2 text-xs'>
      <div className='flex flex-row items-center gap-2'>
        ...
        <Select
          ...
          onValueChange={handleMobileSortColumnChange}
        >
      ...
      {mobileSortColumn && (
        <div className='col-span-1 flex flex-row items-center space-x-2'>
          <Button
            variant={'outline'}
            className='m-0 flex w-fit items-center space-x-1 p-0 px-2'
            onClick={() => handleDirectionToggle(!isDescending)}
          >
            <ArrowUp
              className={`h-5 w-5 ${!isDescending ? 'text-primary' : 'text-muted-foreground'}`}
            />
            <ArrowDown
              className={`h-5 w-5 ${isDescending ? 'text-primary' : 'text-muted-foreground'}`}
            />

...
export const renderMobileCards = <T,>(
  mobileTable: Table<T>,
  ...
  return (
    <div className='flex flex-col gap-4'>
      {mobileTable.getRowModel().rows.length ? (
        mobileTable.getRowModel().rows.map((row: Row<any>) => {
          ...
          return (
            <Card key={row.id} className='p-2 shadow-lg'>
              <CardHeader className='flex flex-col gap-0 p-0 py-0'>
                <div className='flex flex-row items-center justify-between'>
                  ...
                  {showDetail && (
                    <AppSheet title={String(titleValue)} openLabel='Detail'>
                    ...
                    row
                      ...
                      .filter(
                        (cell) =>
                          ...
                          descriptionColumns.includes(
                            cell.column.columnDef.id as any
                          )
                      )
                  )}
                </div>
...
// helpers
const renderHeadless = <T,>(row: Row<any>, columnKeys: (keyof T)[]) => {
  return columnKeys.map((nextToTitle) => {
    ...
    return nextTo ? (
      <span key={nextToTitle as string}>
...
// helpers
const renderForDetail = (cells: Cell<any, unknown>[]) => {
...
// helpers
const renderWithHeader = (cells: Cell<any, unknown>[]) => {
  return cells.map((cell) => {
    const headerStr = String(cell.column.columnDef.header || '');
    let value = flexRender(cell.column.columnDef.cell, cell.getContext());
    if (!value || (typeof value === 'string' && value.length === 0)) {
    ...
    return (
      <div key={cell.id} className='flex flex-wrap items-center gap-1'>

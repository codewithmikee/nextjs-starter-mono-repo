...
export type Period = {
  ...
  startDate: Date;
  endDate: Date;
  days?: number; // Only for LAST_X_DAYS
};

export type BasePeriodFilterProps = {
  value: Period;
...
export const BasePeriodFilter = ({
  value,
  onChange,
  className,
  maxCustomDays = 365
}: BasePeriodFilterProps) => {
  ...
  const handleCustomDateChange = (range: { start?: Date; end?: Date }) => {
  ...
  return (
    <div className={cn('flex items-center gap-2', className)}>
      ...
      {value.option === PeriodOption.LAST_X_DAYS && (
        <input
          type='number'
          min='1'
          max={maxCustomDays}
      ...
      {value.option === PeriodOption.CUSTOM && (

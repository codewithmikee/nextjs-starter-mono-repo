...
function AppFormWrapper({ children, form, submitHandler, onSuccess, onError, buttonProps = defaultButtonProps, layout = 'vertical', disabled = false, title, description, showSuccessMessage = true, successMessage = 'Form submitted successfully' }) {
    ...
    const handleError = (error) => {
        ...
        if (Array.isArray(message)) {
            ...
            message.forEach((errorMsg) => {
                const formFieldKeys = Object.keys(form.getValues()).map((key) => key.toLocaleLowerCase());
                ...
                if (fieldMatch) {
                    const fieldName = fieldMatch[1].toLocaleLowerCase();
    ...
    return (<div className='space-y-2 p-2'>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleSubmit)} className='space-y-4'>
          {status === 'error' && errorMessage && (<AppFormAlert variant='destructive' message={errorMessage}/>)}

          {status === 'success' && showSuccessMessage && (<AppFormAlert variant='default' message={successMessage}/>)}

          {status === 'submitting' ? (<CardSkeleton />) : (<div className={cn('mb-4 space-y-4 px-2', {
                'grid grid-cols-[1fr_2fr] gap-4': layout === 'horizontal'
            })}>
              {children}
            </div>)}

          <FormActions status={status} disabled={disabled} onReset={resetForm} buttonProps={buttonProps}/>
        </form>
      </Form>
    </div>);
}
